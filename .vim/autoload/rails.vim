...
function! s:readable_calculate_file_type() dict abort
  ...
  elseif f =~ '\<app/models/'
    ...
    elseif f =~ '_mailer\.rb$'
      let r = "mailer"
    elseif top =~ '\<\%(validates_\w\+_of\|set_\%(table_name\|primary_key\)\|has_one\|has_many\|belongs_to\|embeds_many\|embeded_in\)\>'
      let r = "model-arb"
    else
      let r = "model"
    endif
  ...
  if (!exists("g:rails_syntax") || g:rails_syntax)
    ...
    if &syntax == 'ruby'
      ...
      if buffer.type_name() ==# 'model' || buffer.type_name('model-arb')
        ...
        syn keyword rubyRailsARAssociationMethod belongs_to has_one has_many embeds_many embedded_in embeds_one has_and_belongs_to_many composed_of accepts_nested_attributes_for
