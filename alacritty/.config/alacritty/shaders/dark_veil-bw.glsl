...
void main()
{
    ...
    float scan = floor(uv.y * resolution.y * 0.18);
    float glitch_gate = step(0.86, hash(vec2(floor(time * 2.5), scan)));
    float glitch_shift = (hash(vec2(scan, floor(time * 2.5))) - 0.5) * 0.018 * glitch_gate;
    p.x += glitch_shift;
    ...
    float z_black = 1.0 - smoothstep(0.28, 0.54, zone);
    float z_gray = smoothstep(0.24, 0.50, zone) * (1.0 - smoothstep(0.58, 0.80, zone));
    float z_white = smoothstep(0.66, 0.88, zone);

    vec3 black = vec3(0.015, 0.015, 0.018);
    vec3 gray = vec3(0.32, 0.32, 0.36);
    ...
    float banding = 0.82 + 0.18 * sin((p.y + h * 0.3) * 42.0 + time * 0.45);
    float shading = 0.28 + 0.42 * body + 0.34 * folds + 0.18 * diffuse;
    vec3 col = palette * shading * banding;
    col = mix(col, white, sheen * 0.12 + fresnel * 0.04);
    col *= 0.74 + 0.28 * pockets;

    float vignette = smoothstep(1.22, 0.16, length(p));
    float alpha_wave = 0.86 + 0.14 * sin(time * 0.26 + h * 2.8);
