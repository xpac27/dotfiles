...
void main()
{
    ...
    vec3 red = vec3(0.70, 0.10, 0.10);
    vec3 yellow = vec3(0.88, 0.74, 0.20);
    vec3 white = vec3(0.96, 0.95, 0.92);

    vec3 palette = red * z_red + yellow * z_yellow + white * z_white;
    float palette_weight = max(z_red, max(z_yellow, z_white));
    palette = mix(red, palette, clamp(palette_weight, 0.0, 1.0));
