...
void main()
{
    ...
    vec3 deep = vec3(0.090, 0.024, 0.030);
    vec3 low = vec3(0.250, 0.060, 0.050);
    vec3 blood = vec3(0.540, 0.120, 0.095);
    vec3 warm = vec3(0.720, 0.320, 0.140);
    vec3 pale = vec3(0.940, 0.910, 0.860);

    float warmth = smoothstep(0.25, 0.95, h + 0.12 * diffuse);
    float pale_mix = clamp(0.10 * fresnel + sheen * (0.18 + folds * 0.22), 0.0, 1.0);

    vec3 col = mix(deep, low, body);
    col = mix(col, blood, folds * 0.88);
    col = mix(col, warm, warmth * (0.22 + 0.40 * diffuse));
    col = mix(col, pale, pale_mix);
    col *= 0.88 + 0.22 * pockets;
